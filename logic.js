import { sb } from "./supabase.js";
import { kuvaKasutajaNimi, laeRoll } from "./auth.js";
import { laeSeaded } from "./seaded.js";

// DOM elemendid
const kuuValik = document.getElementById("kuuValik");
const lukustaNupp = document.getElementById("lukustaNupp");
const salvestaNupp = document.getElementById("salvestaNupp");
const arhiiviNupp = document.getElementById("arhiiviNupp");
const prindiNupp = document.getElementById("prindiNupp");
const laeAllaNupp = document.getElementById("laeAllaNupp");
const vaateReziim = document.getElementById("vaateReziim");
const tbody = document.getElementById("tbody");
const tabelHead = document.getElementById("tabelHead");
const tabelFoot = document.getElementById("tabelFoot");

let seaded = null;
let tabelLukus = true;
let praeguneKuu = null;

// =========================
//  INIT
// =========================

async function initKassatabel() {
    await kuvaKasutajaNimi();

    seaded = await laeSeaded();

    await laeKuuValikud();
    praeguneKuu = kuuValik.value;

    await genereeriKuuTabel();
    const andmed = await laeKuuAndmedSupabasest(praeguneKuu);
    täidaTabelSupabaseAndmetega(andmed);

    arvutaKuuKokku();
    rakendaLukustusOlek(true);
}

// =========================
//  KUU VALIKUD
// =========================

async function laeKuuValikud() {
    const { data } = await sb.from("kassatabel").select("kuu_id").order("kuu_id");

    const kuud = [...new Set(data.map(r => r.kuu_id))];

    kuuValik.innerHTML = kuud
        .map(k => `<option value="${k}">${k}</option>`)
        .join("");

    kuuValik.onchange = vahetaKuud;
}

// =========================
//  KUU VAHETUS
// =========================

async function vahetaKuud() {
    praeguneKuu = kuuValik.value;

    await genereeriKuuTabel();
    const andmed = await laeKuuAndmedSupabasest(praeguneKuu);
    täidaTabelSupabaseAndmetega(andmed);

    tabelLukus = false;
    rakendaLukustusOlek(false);
    arvutaKuuKokku();
}

// =========================
//  SUPABASE PÄRINGUD
// =========================

async function laeKuuAndmedSupabasest(kuuId) {
    const { data } = await sb
        .from("kassatabel")
        .select("*")
        .eq("kuu_id", kuuId)
        .order("kuupaev");

    return data || [];
}

async function salvestaSupabasse(rida) {
    await sb.from("kassatabel").upsert(rida, { onConflict: "kuu_id,kuupaev" });
}

async function arhiiviSupabasse(kuuId, stateJson) {
    await sb.from("arhiiv").insert({ kuu_id: kuuId, state: stateJson });
}

// =========================
//  TABELI GENEREERIMINE
// =========================

async function genereeriKuuTabel() {
    tabelHead.innerHTML = `
        <tr>
            <th>Kuupäev</th>
            ${seaded.veerud.map(v => `<th>${v.pealkiri}</th>`).join("")}
        </tr>
    `;

    tbody.innerHTML = "";

    for (let i = 1; i <= 31; i++) {
        const kuupaev = `${praeguneKuu}-${String(i).padStart(2, "0")}`;
        const tr = document.createElement("tr");
        tr.dataset.date = kuupaev;

        tr.innerHTML = `
            <td>${kuupaev}</td>
            ${seaded.veerud
                .map(v => `<td><input data-veeru-nimi="${v.nimi}" data-veeru-tüüp="${v.tüüp}" /></td>`)
                .join("")}
        `;

        tbody.appendChild(tr);
    }

    tabelFoot.innerHTML = `
        <tr>
            <td colspan="${seaded.veerud.length + 1}">
                <strong>Kuu kokku:</strong> <span id="kuuKokku"></span>
            </td>
        </tr>
    `;
}

// =========================
//  TABELI TÄITMINE
// =========================

function täidaTabelSupabaseAndmetega(andmed) {
    andmed.forEach(r => {
        const tr = tbody.querySelector(`tr[data-date="${r.kuupaev}"]`);
        if (!tr) return;

        const inputs = tr.querySelectorAll("input");
        inputs.forEach(inp => {
            const nimi = inp.dataset.veeruNimi;
            if (r[nimi] !== undefined) inp.value = r[nimi];
        });
    });
}

// =========================
//  KUU KOKKU
// =========================

function arvutaKuuKokku() {
    let summa = 0;

    tbody.querySelectorAll("tr").forEach(tr => {
        tr.querySelectorAll("input").forEach(inp => {
            const tüüp = inp.dataset.veeruTüüp;
            if (tüüp === "number" || tüüp === "toit") {
                summa += Number(inp.value) || 0;
            }
        });
    });

    document.getElementById("kuuKokku").textContent = summa.toFixed(2);
}

// =========================
//  LUKUSTUS
// =========================

function rakendaLukustusOlek(lukus) {
    tbody.querySelectorAll("input").forEach(inp => {
        inp.disabled = lukus;
    });

    lukustaNupp.textContent = lukus
        ? "Tabel lukus (ava sisestamiseks)"
        : "Tabel avatud (lukusta)";
}

// =========================
//  SALVESTAMINE
// =========================

async function salvestaAktiivneKuu() {
    if (tabelLukus) return alert("Tabel on lukus!");

    const kuuId = praeguneKuu;

    for (let tr of tbody.querySelectorAll("tr")) {
        const kuupaev = tr.dataset.date;
        const rida = { kuu_id: kuuId, kuupaev };

        tr.querySelectorAll("input").forEach(inp => {
            const nimi = inp.dataset.veeruNimi;
            const tüüp = inp.dataset.veeruTüüp;

            if (tüüp === "number" || tüüp === "toit") {
                rida[nimi] = Number(inp.value) || 0;
            } else {
                rida[nimi] = inp.value || "";
            }
        });

        await salvestaSupabasse(rida);
    }

    tabelLukus = true;
    rakendaLukustusOlek(true);
    alert("Salvestatud.");
}

// =========================
//  ARHIIV
// =========================

async function salvestaArhiivi() {
    if (tabelLukus) return alert("Tabel on lukus!");

    const kuuId = praeguneKuu;

    const state = {
        kuu: kuuId,
        veerud: seaded.veerud,
        rows: [],
        kuuKokku: document.getElementById("kuuKokku").textContent
    };

    tbody.querySelectorAll("tr").forEach(tr => {
        const rida = { kuupaev: tr.dataset.date };

        tr.querySelectorAll("input").forEach(inp => {
            const nimi = inp.dataset.veeruNimi;
            const tüüp = inp.dataset.veeruTüüp;

            if (tüüp === "number" || tüüp === "toit") {
                rida[nimi] = Number(inp.value) || 0;
            } else {
                rida[nimi] = inp.value || "";
            }
        });

        state.rows.push(rida);
    });

    await arhiiviSupabasse(kuuId, JSON.stringify(state));
    alert("Arhiivi salvestatud.");
}

// =========================
//  NUPUD
// =========================

lukustaNupp.onclick = () => {
    tabelLukus = !tabelLukus;
    rakendaLukustusOlek(tabelLukus);
};

salvestaNupp.onclick = salvestaAktiivneKuu;
arhiiviNupp.onclick = salvestaArhiivi;
prindiNupp.onclick = () => window.print();
laeAllaNupp.onclick = () => alert("PDF tuleb tulevikus");

// Käivita
initKassatabel();









