...
async function arhiiviSupabasse(kuuId, stateJson) {
    const { error } = await sb
        .from("arhiiv")
        .insert({ kuu_id: kuuId, state: stateJson });

    if (error) console.error("Arhiivi salvestamise viga:", error);
}
    ...
    andmed.forEach(rida => {
        ...
        rows.forEach(row => {
            if (row.dataset.date === supaDate) {
                ...
                // Leia kõik inputid reas
                const inputs = row.querySelectorAll("input");

                inputs.forEach(inp => {
                    const veeruNimi = inp.dataset.veeruNimi;

                    // Kui Supabase rida sisaldab seda välja → täida
...
function arvuta() {
    ...
    // Kogus- ja hinnasummade kogumine dünaamiliselt
    const kogused = {};
    const hinnasummad = {};

    // Algväärtusta kõik veerud nullidega
    veerud.forEach(v => {
        kogused[v.nimi] = 0;
    ...
    rows.forEach(row => {
        ...
        inputs.forEach(inp => {
            ...
            // Kogus läheb alati kokkuvõttesse (kui tüüp on number või toit)
            if (veeruInfo.tüüp === "toit" || veeruInfo.tüüp === "number") {
                kogused[veeruNimi] += kogus;
            }

            // Raha arvutame ainult toiduveergude puhul
    ...
    // --- KOGUS- JA HINNASUMMADE KUVAMINE ---
    veerud.forEach((v, idx) => {
...
// --- LUKUSTUS (dünaamiline) ---
function rakendaLukustusOlek() {
    const kõikInputid = tbody.querySelectorAll("input");

    // Kui tabel on lukus (kas käsitsi, rolli tõttu või arhiivivaates)
    if (tabelLukus) {
        ...
        kõikInputid.forEach(inp => {
            ...
            // ainult toiduveergude koguslahtritel oli roheline taust
        ...
        kõikInputid.forEach(inp => {
            ...
            // roheline koguslahter ainult sisestatavate veergude puhul
            if (inp.dataset.veeruTüüp === "toit" || inp.dataset.veeruTüüp === "number") {
...
salvestaNupp.addEventListener("click", async () => {
    ...
    for (let row of rows) {
        ...
        // Loome dünaamilise objekti
        const rida = {
            kuu_id: kuuId,
            kuupaev
        };

        // Täidame selle vastavalt veergude nimedele
        inputs.forEach(inp => {
            const veeruNimi = inp.dataset.veeruNimi;
            ...
            // Number- ja toiduveerud → number
            if (veeruTüüp === "toit" || veeruTüüp === "number") {
                rida[veeruNimi] = Number(inp.value) || 0;
            }

            // Tekstiveerud → tekst
            else if (veeruTüüp === "tekst" || veeruTüüp === "kommentaar") {
...
arhiiviNupp.addEventListener("click", async () => {
    ...
    const state = {
        kuu: kuuId,
        veerud: seaded.veerud, // salvestame ka struktuuri!
        rows: [],
        kuuKokku: document.getElementById("kuuKokku").textContent
    };

    // Läbime kõik tabeli read
...
async function kuvaArhiiv() {
    ...
    // Kontroll: kas arhiivis on read?
    if (!state.rows || !Array.isArray(state.rows)) {
        console.warn("Arhiivi kirjes puudub rows:", state);
        arhiiviKuva.style.display = "none";
        return;
    }

    // Kontroll: kas arhiivis on veerude struktuur?
    if (!state.veerud || !Array.isArray(state.veerud)) {
        console.warn("Arhiivi kirjes puudub veerude struktuur:", state);
        arhiiviKuva.style.display = "none";
        return;
    }
    ...
    const tbodyHtml = state.rows.map(r => `
        <tr>
            <td>${r.kuupaev}</td>
            ${veerud.map(v => `
                <td>${r[v.nimi] !== undefined ? r[v.nimi] : ""}</td>
            `).join("")}
        </tr>
    `).join("");
...
kuuValik.addEventListener("change", async () => {
    ...
    // Loo tabel dünaamiliste veergudega
    await genereeriKuuTabel();

    // Lae kuu andmed Supabasest
    const andmed = await laeKuuAndmedSupabasest(uusKuu);
    täidaTabelSupabaseAndmetega(andmed);

    // Kuva arhiiv (kui olemas)
    await kuvaArhiiv();

    // Uuenda visuaalne režiim
    uuendaVaateReziim();
...
function uuendaVaateReziim() {
    ...
    if (!box) return; // ← lisa see, et vältida tulevasi vigu
...
(async () => {
    ...
    // Kui URL-is on ?paranda=2025-12 → käivitame parandamisrežiimi
...
// --- ARHIIVI PARANDAMISE REŽIIM ---
async function käivitaArhiiviParandus(kuuId) {
    const arhiiv = await laeArhiiv(kuuId);
    if (!arhiiv) {
        alert("Arhiivi ei leitud.");
    ...
    if (typeof state === "string") {
        try {
            ...
            console.error("Arhiivi JSON parse error:", e);
            alert("Arhiivi ei saanud laadida.");
            return;
        }
    }

    if (!state || !Array.isArray(state.veerud) || !Array.isArray(state.rows)) {
        console.error("Arhiivi state struktuur ootamatu:", state);
        alert("Arhiivi struktuur vigane.");
        return;
    }

    // Algväärtusta seaded, kui pole veel olemas
    if (!seaded) seaded = {};

    // Laeme arhiivi struktuuri
    seaded.veerud = state.veerud;

    // Genereerime tabeli
    await genereeriKuuTabel();

    // Täidame read
    täidaTabelSupabaseAndmetega(state.rows);

    // Avame tabeli muutmiseks
    tabelLukus = false;
    rakendaLukustusOlek();
    ...
    if (box) {
        box.style.display = "block";
        box.textContent = `Arhiivi parandamine: ${kuuId}`;
        box.classList.add("parandus");
    }

    // Ülekirjutame salvestusnupu käitumise
if (salvestaNupp) {
    salvestaNupp.onclick = async () => {
        ...
        tbody.querySelectorAll("tr").forEach(row => {
            ...
            inputs.forEach(inp => {
                const veeruNimi = inp.dataset.veeruNimi;
                if (veeruNimi) rida[veeruNimi] = inp.value;
            });

            uusState.rows.push(rida);
        });

        // --- SIIN lisa kasutaja veerg ---
        await sb.from("arhiiv").insert({
            kuu_id: kuuId,
            state: JSON.stringify(uusState),
            kasutaja: window.currentUser?.email || "tundmatu"
        });

        alert("Arhiiv uuendatud.");
        window.location = "arhiiv.html";
    };
}
